...
.patient-card { background-color: #f8f9fa; border: 1px solid #dee2e6; border-radius: 8px; padding: 1rem; margin: 0.5rem 0; }
...
.danger-box { background-color: #FFEBEE; border-left: 4px solid #F44336; padding: 1rem; border-radius: 4px; margin: 1rem 0; }
.warning-box { background-color: #FFF3E0; border-left: 4px solid #FF9800; padding: 1rem; border-radius: 4px; margin: 1rem 0; }
.success-box { background-color: #E8F5E9; border-left: 4px solid #4CAF50; padding: 1rem; border-radius: 4px; margin: 1rem 0; }
.info-box { background-color: #E3F2FD; border-left: 4px solid #1976D2; padding: 1rem; border-radius: 4px; margin: 1rem 0; }
</style>
...
def vstat(val, typ):
    if typ == "bp_sys":
        if val < 90 or val > 160: return "danger"
...
def diagnose(symptoms, vitals):
    ...
    if vitals.get("bp_sys") and vitals["bp_sys"] > 140: d.append(("High BP", 60, ["ECG", "Kidney Test"]))
...
def sidebar():
    with st.sidebar:
        ...
        menu = st.radio("Go to:", [
            "🏠 Dashboard", "👤 New Patient", "🔍 Search Patient",
            "🩺 Vitals", "🤒 Symptoms", "💊 Prescription",
            "📊 Monitoring", "📅 Appointments", "📚 Medicines", "🆘 Emergency"
        ])
        st.markdown("---")
...
def dashboard():
    ...
    c2.markdown(f'<div class="metric-card" style="background:linear-gradient(135deg,#f093fb,#f5576c)"><div class="metric-value">{vt}</div><div>Vitals Today</div></div>', unsafe_allow_html=True)
    c3.markdown(f'<div class="metric-card" style="background:linear-gradient(135deg,#4facfe,#00f2fe)"><div class="metric-value">{pt}</div><div>Prescriptions</div></div>', unsafe_allow_html=True)
    c4.markdown(f'<div class="metric-card" style="background:linear-gradient(135deg,#43e97b,#38f9d7)"><div class="metric-value">{at}</div><div>Appointments</div></div>', unsafe_allow_html=True)
    st.markdown("---")
        ...
        if st.form_submit_button("✅ Register"):
            if not name:
                ...
                c.execute("INSERT INTO patients VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                         (pid, hid, name, age, gender, phone, address, aadhaar,
                          bg if bg!="Select" else None, allergies, history, cat, datetime.now()))
                conn.commit()
...
def symptom_checker():
    ...
    with st.form("symptoms"):
        ...
        if st.form_submit_button("🔍 Analyze"):
            ...
            vd = {}
            if lv:
                vd = {'bp_sys': lv[2], 'bp_dia': lv[3], 'pulse': lv[4], 'spo2': lv[5], 'temp': lv[6], 'bs_fasting': lv[9]}
            dx = diagnose(sym, vd)
...
def monitoring():
    ...
    c1.markdown(f"**BP**<br><span class='vital-{vstat(latest[2],'bp_sys')}'>{latest[2]}/{latest[3]}</span>", unsafe_allow_html=True)
    c2.markdown(f"**Pulse**<br><span class='vital-{vstat(latest[4],'pulse')}'>{latest[4]}</span>", unsafe_allow_html=True)
    c3.markdown(f"**SpO2**<br><span class='vital-{vstat(latest[5],'spo2')}'>{latest[5]}%</span>", unsafe_allow_html=True)
    c4.markdown(f"**Temp**<br><span class='vital-{vstat(latest[6],'temp')}'>{latest[6]}F</span>", unsafe_allow_html=True)
